...
async def back(callback: CallbackQuery):
    ...
    if info['category'] in ('Товары', 'О нас', 'Оплата', 'Доставка', 'Корзина'):
        info['category'] = 'Главное меню'
    else:
...
@router.callback_query(or_f(F.data == 'Пицца', F.data == 'Ролотто', F.data == 'Напитки', F.data == 'Корзина'))
async def callback_food(callback: CallbackQuery):
    ...
    if info[f'{pref}_captions']:
        info['current_item'] = info[f'{pref}_captions'][0]
        category = callback.data
        info['category'] = category
        data = buttons[info['category']]
        caption = '\n'.join(text for text in info[f'{pref}_captions'][0].values() if not text.startswith('https'))
...
async def pagination_handler(callback: CallbackQuery, callback_data: kb.Pagination):
    ...
    caption = '\n'.join(text for text in info[f'{pref}_captions'][page].values() if not text.startswith('https'))
